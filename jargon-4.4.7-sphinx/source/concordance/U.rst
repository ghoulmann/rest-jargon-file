...
Compare `pilot error <./P.html#pilot-error>`__,
`PEBKAC <./P.html#PEBKAC>`__, `ID10T <./I.html#idiot>`__; oppose
`PBD <./P.html#PBD>`__; see also
...
email `spam <./S.html#spam>`__. Compare `UCE <UCE>`__. The UBE
...
`geek <./G.html#geek>`__] Almost synonymous with
`demigod <./D.html#demigod>`__; used as a compliment of someone
regarded as a paragon of `geek <./G.html#geek>`__ achievement and
virtue. Has partially replaced earlier
`demigod <./D.html#demigod>`__.
...
for email `spam <./S.html#spam>`__. Compare `UBE <UBE>`__,
...
Penalty <Usenet-Death-Penalty>`__. Common (probably now more so
...
Written shorthand for `micro- <./M.html#micro->`__; techspeak when
...
ugly ™ typography (see also `(TM) <./other.html#TM>`__). Also used to
...
`glob <./G.html#glob>`__ and `splat out <./S.html#splat-out>`__.
...
enable the listener to `grok <./G.html#grok>`__ it. “Let's now look
...
`nontrivial <./N.html#nontrivial>`__, can be solved simply by
...
`toolsmith <./T.html#toolsmith>`__) generalize uninteresting problems
...
tectonic plate). See `WOMBAT <./W.html#WOMBAT>`__,
`SMOP <./S.html#SMOP>`__; compare `toy
problem <./T.html#toy-problem>`__, oppose
...
An example of Unix brain damage is a `kluge <./K.html#kluge>`__ in a
...
`ITS <./I.html#ITS>`__ and `TOPS-20 <./T.html#TOPS-20>`__ fans,
...
referenced in the `back door <./B.html#back-door>`__ entry.

In this view, Unix was designed to be one of the first computer viruses
(see `virus <./V.html#virus>`__) — but a virus spread to computers
...
by `DEC <./D.html#DEC>`__ president Kenneth Olsen shortly before DEC
...
around the same time `Linux <./L.html#Linux>`__ and other free-Unix
...
1996 the variant called `Linux <./L.html#Linux>`__ has been at the
cutting edge of the `open source <./O.html#open-source>`__ movement.
...
weenie <Unix-weenie>`__ and `Unix
conspiracy <Unix-conspiracy>`__ for an opposing point of view).
See `Version 7 <./V.html#Version-7>`__, `BSD <./B.html#BSD>`__,
...
and `troff <./T.html#troff>`__ had just been invented and we were
...
`unixism <unixism>`__\ s include: gratuitous use of fork(2); the
...
`obscure <./O.html#obscure>`__ side-effects of system calls (use of
...
`vaxocentrism <./V.html#vaxocentrism>`__; see also `New
Jersey <./N.html#New-Jersey>`__.
...
`Weenix <./W.html#Weenix>`__, `Unix
conspiracy <Unix-conspiracy>`__. See also
`weenie <./W.html#weenie>`__.
...
course, called a `download <./D.html#download>`__
...
Earlier in the discussion (see `thread <./T.html#thread>`__), i.e.,
‘above’. “As Joe pointed out upthread, ...” See also
`followup <./F.html#followup>`__.
...
See `munchkin <./M.html#munchkin>`__.
...
`spam <./S.html#spam>`__. This can be either passive or active. A
...
Compare `Internet Death Penalty <./I.html#Internet-Death-Penalty>`__,
...
`bboard <./B.html#bboard>`__ (bulletin board) system supported mainly
...
`newsgroup <./N.html#newsgroup>`__\ s and an unguessably huge volume
of new technical articles, news, discussion, chatter, and
`flamage <./F.html#flamage>`__ every day (and that leaves out the
graphics...).
...
done. See `menuitis <./M.html#menuitis>`__, `drool-proof
paper <./D.html#drool-proof-paper>`__,
`Macintrash <./M.html#Macintrash>`__,
`user-obsequious <user-obsequious>`__.
...
`luser <./L.html#luser>`__\ s. The users are looked down on by
...
Emphatic form of `user-friendly <user-friendly>`__. Connotes a
...
Compare `lenna <./L.html#lenna>`__, `Stanford
...
`RTFS <./R.html#RTFS>`__ (sense 1), but more polite. This is a common
...
`wizard <./W.html#wizard>`__\ s to answer them.

Once upon a time in `elder days <./E.html#elder-days>`__, everyone
...
`cat <./C.html#cat>`__; the reference is to the Unix command cat(1),
...
page <http://www.ling.helsinki.fi/~reriksso/unix/award>`__ devoted
