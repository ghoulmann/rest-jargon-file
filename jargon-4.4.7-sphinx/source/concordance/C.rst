...
Syn. for `bitty box <./B.html#bitty-box>`__.
...
Compare `BiCapitalization <./B.html#BiCapitalization>`__; but where
...
See `PascalCasing <./P.html#PascalCasing>`__.
...
are likely to cause you to `snarf <./S.html#snarf>`__ coffee onto
...
1. Mythically, a `robocanceller <./R.html#robocanceller>`__
...
`spam <./S.html#spam>`__-fighters. Once upon a time, the 'Moose would
...
sugar <./S.html#syntactic-sugar>`__; the term is also a play on
‘candygram’. `COBOL <COBOL>`__, Apple's Hypertalk language, and a
...
Calculus <./K.html#Knights-of-the-Lambda-Calculus>`__). Compare
`vanilla <./V.html#vanilla>`__.
...
code <./F.html#firewall-code>`__ (sense 2).
...
A variety of `shareware <./S.html#shareware>`__ for which either the
...
Syn.: `charityware <charityware>`__; compare
`crippleware <crippleware>`__, sense 2.
...
(compare `shotgun debugging <./S.html#shotgun-debugging>`__, `voodoo
...
the new message; an `include war <./I.html#include-war>`__ in which
...
The addition of a new `feature <./F.html#feature>`__ to an existing
...
`software bloat <./S.html#software-bloat>`__.
...
At `DEC <./D.html#DEC>`__ (now HP), this is sometimes called
...
`overclocking <./O.html#overclock>`__ (e.g. with cutouts for
...
What a `guru <./G.html#guru>`__ does when you ask him or her to run a
...
`runes <./R.html#runes>`__, `examining the
entrails <./E.html#examining-the-entrails>`__; also see the AI koan
about Tom Knight in `Some AI Koans <../koans>`__ (in Appendix A).
...
which the `field circus <./F.html#field-circus>`__ had given up on.
...
`wedged <./W.html#wedged>`__ or `hung <./H.html#hung>`__ that it
...
middle of a winning game of `nethack <./N.html#nethack>`__ and it
went catatonic on me! Aaargh!” Compare `buzz <./B.html#buzz>`__.
...
[see `bazaar <./B.html#bazaar>`__ for derivation] The ‘classical’
mode of software engineering long thought to be necessarily implied by
`Brooks's Law <./B.html#Brookss-Law>`__. Features small teams, tight
...
[from catenate via `Unix <./U.html#Unix>`__ cat(1)]

1. [techspeak] To spew an entire file to the screen or some other output
sink without pause (syn. `blast <./B.html#blast>`__).
...
Rare outside Unix sites. See also `dd <./D.html#dd>`__,
`BLT <./B.html#BLT>`__.
...
`canonical <canonical>`__ example of *bad* user-interface design,
because of its woefully unobvious name. It is far more often used to
`blast <./B.html#blast>`__ a file to standard output than to
concatenate two files. The name **cat** for the former operation is just
as unintuitive as, say, LISP's `cdr <cdr>`__.

Of such oppositions are `holy wars <./H.html#holy-wars>`__ made....
See also `UUOC <./U.html#UUOC>`__.
...
page <./H.html#home-page>`__\ s black for 48 hours. Several
...
through <./L.html#loop-through>`__.
...
wastebasket), for collecting the `chad <chad>`__ (sense 2) that
accumulated in `Iron Age <./I.html#Iron-Age>`__ card punches. You had
to open the covers of the card punch periodically and empty the chad
box. The `bit bucket <./B.html#bit-bucket>`__ was notionally the
...
`selvage <./S.html#selvage>`__, `perf <./P.html#perf>`__, and
`ripoff <./R.html#ripoff>`__.
...
the earmarks of a `backronym <./B.html#backronym>`__. It has also
...
This is *one* way to be `chad <chad>`__\ less.
...
a child or successor without going through the `OS <./O.html#OS>`__
...
`exec <./E.html#exec>`__. Oppose the more modern subshell.
...
`muggle <./M.html#muggle>`__. Implies both ignorance and a certain
amount of willingness to learn, but does not necessarily imply as little
experience or short exposure time as `newbie <./N.html#newbie>`__ and
is not as derogatory as `luser <./L.html#luser>`__. Both a novice
...
`IRC <./I.html#IRC>`__, or a GEnie chat board, just as a social
...
[IRC] The basic unit of discussion on `IRC <./I.html#IRC>`__. Once
...
`IRC <./I.html#IRC>`__ channel; commonly abbreviated chanop or CHOP
or just op (as of 2000 these short forms have almost crowded out the
parent usage). These privileges include the right to
`kick <./K.html#kick>`__ users, to change various status bits, and to
...
[IRC] See `channel op <channel-op>`__.
...
pointer <./D.html#dangling-pointer>`__ and
`snap <./S.html#snap>`__.

2. [Cambridge] pointer chase or pointer hunt: The process of going
through a `core dump <core-dump>`__ (sense 1), interactively or
on a large piece of paper printed with hex
`runes <./R.html#runes>`__, following dynamic data-structures. Used
...
similar terms, see `nybble <./N.html#nybble>`__.
...
`molly-guard <./M.html#molly-guard>`__\ s).
...
`number-crunching <./N.html#number-crunching>`__ when you'd far
...
`life <./L.html#life>`__ patterns. May or may not refer to someone
...
See `laser chicken <./L.html#laser-chicken>`__.
...
storm <./B.html#broadcast-storm>`__ and/or `network
meltdown <./N.html#network-meltdown>`__, in memory of the April 1986
...
tree packet <Christmas-tree-packet>`__.
...
exception of the `Amiga <./A.html#Amiga>`__, Commodore's machines
were notoriously crocky little `bitty
box <./B.html#bitty-box>`__\ es, albeit people have written
...
technique <./M.html#Mongolian-Hordes-technique>`__.
...
lpr(1) choke.” “I tried building an `EMACS <./E.html#EMACS>`__ binary
to use `X <./X.html#X>`__, but cpp(1) choked on all those
**#define**\ s.” See `barf <./B.html#barf>`__,
`vi <./V.html#vi>`__.
...
`loser <./L.html#loser>`__, `bagbiter <./B.html#bagbiter>`__,
`chomp <chomp>`__.
...
1. To `lose <./L.html#lose>`__; specifically, to chew on something of
which more was bitten off than one can. Probably related to gnashing of
teeth.

2. To bite the bag; See `bagbiter <./B.html#bagbiter>`__.
...
Doubling <../verb-doubling>`__ in the `Jargon
Construction <../construction>`__ section of the Prependices). The
...
See `kamikaze packet <./K.html#kamikaze-packet>`__, `Chernobyl
packet <Chernobyl-packet>`__. (The term doubtless derives from a
...
whistles <./B.html#bells-and-whistles>`__ by the fact that the latter
...
`Unix <./U.html#Unix>`__; so called because many features derived
...
designing `C++ <C-plus-plus>`__, there was a humorous debate over
...
choice <./L.html#languages-of-choice>`__, `indent
...
To run slowly; to `grind <./G.html#grind>`__ or
`grovel <./G.html#grovel>`__. “The disk is chugging like crazy.”
...
A mutant offshoot of `Discordianism <./D.html#Discordianism>`__
...
`slack <./S.html#slack>`__. There is a home page at
...
pulled on the rope. See also `book titles <./B.html#book-titles>`__.
...
block <./S.html#sig-block>`__\ s just before a CompuServe address.
Syn. `Compu$erve <CompuServe>`__.
...
first edition of `K&R <./K.html#K-ampersand-R>`__, with some small
...
array and clobbered the stack.” Compare `mung <./M.html#mung>`__,
`scribble <./S.html#scribble>`__, `trash <./T.html#trash>`__, and
...
discussing the instruction set. Compare `cycle <cycle>`__,
`jiffy <./J.html#jiffy>`__.
...
[DEC] Syn. `case and paste <case-and-paste>`__.
...
`Linux <./L.html#Linux>`__ and the free BSDs killed off this product
...
copy of that file before you `mung <./M.html#mung>`__ it”.
...
[Mac users] See `feature key <./F.html#feature-key>`__.
...
clue-by-four is a close relative of the `LART <./L.html#LART>`__.
...
`snarf <./S.html#snarf>`__\ ed my coffee onto my keyboard.”. Common
on "alt.fan.pratchett" and `scary devil
monastery <./S.html#scary-devil-monastery>`__; recognized elsewhere.
The `Acronymphomania
FAQ <http://www.lspace.org/faqs/acronym-faq.g>`__ on
...
produce a high proportion of `coasters <coaster>`__.
...
`candygrammar <candygrammar>`__); thus it is alleged that
...
`evil <./E.html#evil>`__.) A weak, verbose, and flabby language used
by `code grinder <code-grinder>`__\ s to do boring mindless
things on `dinosaur <./D.html#dinosaur>`__ mainframes. Hackers
believe that all COBOL programmers are `suit <./S.html#suit>`__\ s or
`code grinder <code-grinder>`__\ s, and no self-respecting hacker
...
loathing <./F.html#fear-and-loathing>`__, `software
rot <./S.html#software-rot>`__.
...
1. A `suit <./S.html#suit>`__-wearing minion of the sort hired in
...
grinder <code-grinder>`__'s milieu is about as far from hackerdom
as one can get and still touch a computer; the term connotes pity. See
`Real World <./R.html#Real-World>`__, `suit <./S.html#suit>`__.
...
technique, rule-boundedness, `brute force <./B.html#brute-force>`__,
and utter lack of imagination.

Contrast `hacker <./H.html#hacker>`__, `Real
Programmer <./R.html#Real-Programmer>`__.
...
sort <./B.html#bubble-sort>`__?”, or “The code is loaded at the high
...
`newbie <./N.html#newbie>`__ or a `suit <./S.html#suit>`__.
...
`management <./M.html#management>`__ decision, or of complaining
...
see `geek code <./G.html#geek-code>`__.
...
`weenie <./W.html#weenie>`__\ s. “Dike out that goto or the code
...
`number-crunching <./N.html#number-crunching>`__, rare to unknown
...
The canonical example is `fudge factor <./F.html#fudge-factor>`__.
...
keyboard <./S.html#space-cadet-keyboard>`__, cokebottle faded away as
...
`bang <./B.html#bang>`__). Since the exclamation point looks a lot
...
`automagically <./A.html#automagically>`__ be transferred to the
...
`Datamation <./D.html#Datamation>`__ issue (and was reprinted in the
April 1984 issue of *Communications of the ACM*). This parodied the
then-raging ‘structured programming’ `holy
wars <./H.html#holy-wars>`__ (see `considered
harmful <considered-harmful>`__). Mythically, some variants are
...
contemplate the **ALTER** verb in `COBOL <COBOL>`__. **COME
FROM** was supported under its own name for the first time 15 years
later, in C-INTERCAL (see `INTERCAL <./I.html#INTERCAL>`__,
`retrocomputing <./R.html#retrocomputing>`__); knowledgeable
...
[Mac users] Syn. `feature key <./F.html#feature-key>`__.
...
Compare `condition out <condition-out>`__, usually the preferred
technique in languages (such as `C <C>`__) that make it possible.
...
`newsgroup <./N.html#newsgroup>`__ names (especially two-component
names) tend to be pronounced more often (so soc.wibble is /sok dot
wib´l/ rather than /sohsh wib´l/).

Preferred `metasyntactic
variable <./M.html#metasyntactic-variable>`__\ s include
`blurgle <./B.html#blurgle>`__, **eek**, **ook**, **frodo**, and
**bilbo**; `wibble <./W.html#wibble>`__, **wobble**, and in
emergencies **wubble**; **flob**, **banana**, **tom**, **dick**,
**harry**, **wombat**, **frog**, `fish <./F.html#fish>`__,
`womble <./W.html#womble>`__ and so on and on (see
`foo <./F.html#foo>`__, sense 4). Alternatives to verb doubling
...
`calculator <calculator>`__, `chemist <chemist>`__,
`console jockey <console-jockey>`__, `fish <./F.html#fish>`__,
`go-faster stripes <./G.html#go-faster-stripes>`__,
`grunge <./G.html#grunge>`__, `hakspek <./H.html#hakspek>`__,
`heavy metal <./H.html#heavy-metal>`__, `leaky
heap <./L.html#leaky-heap>`__, `lord high
fixer <./L.html#lord-high-fixer>`__, `loose
bytes <./L.html#loose-bytes>`__, `muddie <./M.html#muddie>`__,
`nadger <./N.html#nadger>`__, `noddy <./N.html#noddy>`__,
`psychedelicware <./P.html#psychedelicware>`__, `raster
blaster <./R.html#raster-blaster>`__, `RTBM <./R.html#RTBM>`__,
`seggie <./S.html#seggie>`__, `spod <./S.html#spod>`__, `sun
lounge <./S.html#sun-lounge>`__, `terminal
junkie <./T.html#terminal-junkie>`__, `tick-list
features <./T.html#tick-list-features>`__,
`weeble <./W.html#weeble>`__, `weasel <./W.html#weasel>`__,
`YABA <./Y.html#YABA>`__, and notes or definitions under `Bad
Thing <./B.html#Bad-Thing>`__, `barf <./B.html#barf>`__,
`bogus <./B.html#bogus>`__, `chase
pointers <chase-pointers>`__, `cosmic
rays <cosmic-rays>`__, `crippleware <crippleware>`__,
`crunch <crunch>`__, `dodgy <./D.html#dodgy>`__,
`gonk <./G.html#gonk>`__, `hamster <./H.html#hamster>`__,
`hardwarily <./H.html#hardwarily>`__,
`mess-dos <./M.html#mess-dos>`__, `nybble <./N.html#nybble>`__,
`proglet <./P.html#proglet>`__, `root <./R.html#root>`__,
`SEX <./S.html#SEX>`__, `tweak <./T.html#tweak>`__,
`womble <./W.html#womble>`__, and `xyzzy <./X.html#xyzzy>`__.
...
through accreting `feature <./F.html#feature>`__\ s and
`cruft <cruft>`__ that don't merge cleanly into the overall
design scheme (thus, some fans of `Classic C <Classic-C>`__
...
See `jock <./J.html#jock>`__ (sense 2).
...
[`demoscene <./D.html#demoscene>`__\ ] Finnish-originated slang for
‘competition’. Demo compos are held at a
`demoparty <./D.html#demoparty>`__. The usual protocol is that
...
include `intro <./I.html#intro>`__ compos (4k or 64k demos), music
compos, graphics compos, quick `demo <./D.html#demo>`__ compos (build
...
`crunch <crunch>`__\ ing of a file using a particular C
implementation of compression by Joseph M. Orost et al.: and widely
circulated via `Usenet <./U.html#Usenet>`__; use of
`crunch <crunch>`__ itself in this sense is rare among Unix
...
See `CI$ <CIS>`__. Synonyms CompuSpend and Compu$pend are also
reported.
...
Syn. `chad <chad>`__. [obs.] Though this term was common at one
...
`chad <chad>`__, which was half-forgotten in 1990, has enjoyed a
revival. —ESR]
...
diesel horsepower. See `bitty box <./B.html#bitty-box>`__, `Get a
real computer! <./G.html#Get-a-real-computer->`__,
`toy <./T.html#toy>`__, `crank <crank>`__.
...
one unit of electric charge (see also `bogon <./B.html#bogon>`__). An
...
`canonical <canonical>`__ examples of these directives are **#if
0** (or **#ifdef notdef**, though some find the latter
`bletcherous <./B.html#bletcherous>`__) and **#endif** in C. Compare
`comment out <comment-out>`__.
...
`SEX <./S.html#SEX>`__ but has also been shown to have a high failure
rate as drive mechanisms attempt to access the disk — and can even
fatally frustrate insertion.

2. The protective cladding on a `light pipe <./L.html#light-pipe>`__.
...
`programming fluid <./P.html#programming-fluid>`__ without impeding
typing.
...
`Great Worm <./G.html#Great-Worm>`__ by exploiting a portability bug
...
who aren't `fans <./F.html#fan>`__. “We'd been corresponding on the
...
model of the `PDP-10 <./P.html#PDP-10>`__), none of whose connectors
...
combatability <./B.html#backward-combatability>`__.
...
`turns out <http://www.theregister.co.uk/content/4/26585>`__, the
...
1. The operator's station of a `mainframe <./M.html#mainframe>`__. In
...
the console is just the `tty <./T.html#tty>`__ the system was booted
...
the console can do real graphics or run `X <./X.html#X>`__.
...
See `terminal junkie <./T.html#terminal-junkie>`__.
...
applied to `flamage <./F.html#flamage>`__, it more usually connotes
...
glossies <./F.html#four-color-glossies>`__.) “He gave a talk on the
...
Considered silly. Compare `control-S <control-S>`__.
...
“Resume.” From the ASCII DC1 or `XON <./X.html#XON>`__ character (the
pronunciation /X-on/ is therefore also used), used to undo a previous
`control-S <control-S>`__.
...
`control-O <control-O>`__ in that the person is asked to stop
...
1968 issue of `Datamation <./D.html#Datamation>`__. Compare `SNAFU
principle <./S.html#SNAFU-principle>`__.
...
the reader can use to do various `magic <./M.html#magic>`__ things in
programs. Cookbooks, slavishly followed, can lead one into `voodoo
programming <./V.html#voodoo-programming>`__, but are useful for
hackers trying to `monkey up <./M.html#monkey-up>`__ small programs
...
[Unix, by opposition from `raw mode <./R.html#raw-mode>`__] The
...
the tty driver. Oppose `raw mode <./R.html#raw-mode>`__, `rare
mode <./R.html#rare-mode>`__. This term is techspeak under Unix but
...
`cookie monster <cookie-monster>`__. A correspondent observes “In
...
A collection of `fortune cookie <./F.html#fortune-cookie>`__\ s in a
...
cookie <./M.html#magic-cookie>`__; see also `fortune
cookie <./F.html#fortune-cookie>`__. Now mainstream in the specific
...
An area of memory set aside for storing `cookie <cookie>`__\ s.
...
number <./M.html#magic-number>`__ in the jar. Programs can inquire
...
early (1970s) hacks reported on `TOPS-10 <./T.html#TOPS-10>`__,
`ITS <./I.html#ITS>`__, `Multics <./M.html#Multics>`__, and
elsewhere that would lock up either the victim's terminal (on a
timesharing machine) or the `console <console>`__ (on a batch
`mainframe <./M.html#mainframe>`__), repeatedly demanding “I WANT A
...
`FOAF <./F.html#FOAF>`__) has described these programs as urban
legends (implying they probably never existed) but they existed, all
right, in several different versions. See also
`wabbit <./W.html#wabbit>`__. Interestingly, the term cookie monster
appears to be a `retcon <./R.html#retcon>`__; the original term was
`cookie bear <cookie-bear>`__.
...
implementation of `chrome <chrome>`__, or the stroking of
`suit <./S.html#suit>`__\ s. “I'll get back to him on that feature in
...
copper — or aluminum! Opposed to `light pipe <./L.html#light-pipe>`__
...
scheme disabled. Syn. `copywronged <copywronged>`__.
...
[C64/amiga `demoscene <./D.html#demoscene>`__] A computer party
...
has gradually evolved the `demoparty <./D.html#demoparty>`__ to
replace it.
...
[play on copyright] Syn. for `copybroke <copybroke>`__.
...
`leak <./L.html#leak>`__ — like a cancer, it kills by crowding out
productive tissue.
...
[common `Iron Age <./I.html#Iron-Age>`__ jargon, preserved by Unix]

1. [techspeak] A copy of the contents of `core <core>`__,
...
4. A recapitulation of knowledge (compare `bits <./B.html#bits>`__,
sense 1). Hence, spewing all one knows about a topic (syn. `brain
dump <./B.html#brain-dump>`__), esp. in a lecture or answer to an exam
...
A `core dump <core-dump>`__ lands our hero in hot water.
...
dump <core-dump>`__ and the core image or core file produced by
one are terms in favor. Some varieties of Commonwealth hackish prefer
`store <./S.html#store>`__.
...
Syn. `memory leak <./M.html#memory-leak>`__.
...
7090 at Bell Labs. See `core <core>`__. For information on the
...
Notionally, the cause of `bit rot <./B.html#bit-rot>`__. However,
this is a semi-independent usage that may be invoked as a humorous way
to `handwave <./H.html#handwave>`__ away any minor
`randomness <./R.html#randomness>`__ that doesn't seem worth the
bother of investigating. “Hey, Eric — I just got a burst of garbage on
my `tube <./T.html#tube>`__, where did that come from?” “Cosmic rays,
I guess.” Compare `sunspots <./S.html#sunspots>`__, `phase of the
moon <./P.html#phase-of-the-moon>`__. The British seem to prefer the
...
Syn. `barf <./B.html#barf>`__. Connotes that the program is throwing
...
printable, so it coughed and died.” Compare `die <./D.html#die>`__,
`die horribly <./D.html#die-horribly>`__, `scream and
die <./S.html#scream-and-die>`__.
...
`warez <./W.html#warez>`__, as opposed to a
`server <./S.html#server>`__ who makes them available for download or
a `leech <./L.html#leech>`__ who merely downloads them. Hackers
recognize this term but don't use it themselves, as the act is not part
of their culture. See also `warez d00dz <./W.html#warez-d00dz>`__,
`cracker <cracker>`__, `elite <./E.html#elite>`__.
...
[Sun, from William Gibson's `cyberpunk <cyberpunk>`__ SF] Synonym
for `hacker <./H.html#hacker>`__. It is reported that at Sun this
...
Scott Adams (the creator of `Dilbert <./D.html#Dilbert>`__) but
already appears in the January 1996 version of the `scary devil
monastery <./S.html#scary-devil-monastery>`__ FAQ, and has been traced
back to a 1989 `sig block <./S.html#sig-block>`__. Compare
`hing <./H.html#hing>`__, `grilf <./G.html#grilf>`__,
`filk <./F.html#filk>`__, `newsfroup <./N.html#newsfroup>`__.
...
`C <C>`__. Now one of the `languages of
choice <./L.html#languages-of-choice>`__, although many hackers still
grumble that it is the successor to either Algol 68 or Ada (depending on
generation), and a prime example of `second-system
effect <./S.html#second-system-effect>`__. Almost anything that can be
done in any language can be done in C++, but it requires a `language
lawyer <./L.html#language-lawyer>`__ to know what is and what is not
legal — the design is *almost* too large to hold in even hackers' heads.
Much of the `cruft <cruft>`__ results from C++'s attempt to be
...
[Control Program/Monitor; later `retcon <./R.html#retcon>`__\ ned to
Control Program for Microcomputers] An early microcomputer
`OS <./O.html#OS>`__ written by hacker Gary Kildall for 8080- and
...
`DEC <./D.html#DEC>`__ operating systems such as
`TOPS-10 <./T.html#TOPS-10>`__, OS/8, RSTS, and RSX-11. See
`MS-DOS <./M.html#MS-DOS>`__, `operating
system <./O.html#operating-system>`__.
...
core <./F.html#fandango-on-core>`__. In severe cases of the disease,
...
death <./E.html#eat-flaming-death>`__.
...
defense against journalistic misuse of `hacker <./H.html#hacker>`__
...
stage <./L.html#larval-stage>`__ is expected to have outgrown the
...
the `mundane <./M.html#mundane>`__ reader misled by sensationalistic
...
`losing <./L.html#losing>`__. Some other reasons crackers are looked
down on are discussed in the entries on `cracking <cracking>`__
and `phreaking <./P.html#phreaking>`__. See also
`samurai <./S.html#samurai>`__, `dark-side
hacker <./D.html#dark-side-hacker>`__, and `hacker
ethic <./H.html#hacker-ethic>`__. For a portrait of the typical
teenage cracker, see `warez d00dz <./W.html#warez-d00dz>`__.
...
1. v. To break into a system (compare `cracker <cracker>`__).
...
4. An `exploit <./E.html#exploit>`__.
...
`cracker <cracker>`__ does. Contrary to widespread myth, this
...
`rootkit <./R.html#rootkit>`__ and other automated cracking has
...
`root <./R.html#root>`__ privileges thereby; see
`cracking <cracking>`__.
...
`troff <./T.html#troff>`__) and because (particularly if the standard
Computer Modern fonts are used) it generates vast output files. See
`religious issues <./R.html#religious-issues>`__,
`TeX <./T.html#TeX>`__.
...
`die horribly <./D.html#die-horribly>`__). The construction
crash-and-burn machine is reported for a computer used exclusively for
alpha or `beta <./B.html#beta>`__ testing, or reproducing bugs (i.e.,
...
`system <./S.html#system>`__ (q.v., sense 1), esp. of magnetic disk
drives (the term originally described what happens when the air gap of a
hard disk collapses). “Three `luser <./L.html#luser>`__\ s lost their
...
crashed the OS!” See `down <./D.html#down>`__. Also used transitively
to indicate the cause of the crash (usually a person or a program, or
both). “Those idiots playing `SPACEWAR <./S.html#SPACEWAR>`__ crashed
the system.”

3. vi. Sometimes said of people hitting the sack after a long `hacking
run <./H.html#hacking-run>`__; see `gronk
out <./G.html#gronk-out>`__.
...
deck <./D.html#dusty-deck>`__ or
`gonkulator <./G.html#gonkulator>`__, but connotes that the thing
...
horror....” Compare `WOMBAT <./W.html#WOMBAT>`__.
...
beloved of `management <./M.html#management>`__, they are generally
ignored.
...
`elegant <./E.html#elegant>`__ past the point of diminishing return,
...
the `Real World <./R.html#Real-World>`__. See also `creeping
featurism <creeping-featurism>`__, `second-system
effect <./S.html#second-system-effect>`__,
`tense <./T.html#tense>`__.
...
`chrome <chrome>`__ and `feature <./F.html#feature>`__\ s onto
systems at the expense of whatever elegance they may have possessed when
originally designed. See also `feeping
creaturism <./F.html#feeping-creaturism>`__. “You know, the main
problem with `BSD <./B.html#BSD>`__ Unix has always been creeping
featurism.”
...
`feature <./F.html#feature>`__.) The result is usually a patchwork
...
program, intended to illustrate `GNU <./G.html#GNU>`__ command-line
...
`second-system effect <./S.html#second-system-effect>`__. See also
`creeping elegance <creeping-elegance>`__.
...
Variant of `creeping featurism <creeping-featurism>`__, with its
...
Congenital `loser <./L.html#loser>`__; an obnoxious person; someone
...
disease <./D.html#dread-high-bit-disease>`__ for an example.
Approximate synonyms: `bletcherous <./B.html#bletcherous>`__,
`bagbiting <./B.html#bagbiting>`__, `losing <./L.html#losing>`__,
...
2. [Cambridge] Variety of `guiltware <./G.html#guiltware>`__ that
exhorts you to donate to some charity (compare
`careware <careware>`__, `nagware <./N.html#nagware>`__).
...
`epsilon <./E.html#epsilon>`__ bugs. (This malady has many causes:
`creeping featurism <creeping-featurism>`__, ports to too many
...
the next line. See `newline <./N.html#newline>`__. Under
`Unix <./U.html#Unix>`__ influence this usage has become less common
...
Mel' <../story-of-mel>`__ in Appendix A.) Many crocks have a
tightly woven, almost completely unmodifiable structure. See
`kluge <./K.html#kluge>`__, `brittle <./B.html#brittle>`__. The
...
[proposed, by analogy with `upload <./U.html#upload>`__ and
`download <./D.html#download>`__] To move files between machines on a
...
tends to cause `followup <./F.html#followup>`__ articles to go to
...
`freeware <./F.html#freeware>`__ circulated by user's groups and BBS
systems in the micro-hobbyist world. “Yet *another* set of disk catalog
utilities for `MS-DOS <./M.html#MS-DOS>`__? What crudware!”
...
[very common; back-formation from `crufty <crufty>`__]
...
`hand-hacking <./H.html#hand-hacking>`__).
...
[from `cruft <cruft>`__] The antithesis of craftsmanship.
...
workable. Like vt. `kluge up <./K.html#kluge-up>`__, but more
...
`hack together <./H.html#hack-together>`__, `hack
up <./H.html#hack-up>`__, `kluge up <./K.html#kluge-up>`__,
`crufty <crufty>`__.
...
`canonical <canonical>`__ example is “This is standard old crufty
`DEC <./D.html#DEC>`__ software”. In fact, one fanciful theory of the
...
`frob <./F.html#frob>`__); often one that doesn't fit well into the
scheme of things. “A LISP property list is a good place to store
crufties (or, collectively, `random <./R.html#random>`__ cruft).”
...
Two binary digits; a `quad <./Q.html#quad>`__. Larger than a
`bit <./B.html#bit>`__, smaller than a
`nybble <./N.html#nybble>`__. Considered silly. Syn.
`tayste <./T.html#tayste>`__. General discussion of such terms is
under `nybble <./N.html#nybble>`__.
...
`number-crunching <./N.html#number-crunching>`__.”
...
Contest <./O.html#Obfuscated-C-Contest>`__ entries are often crunched;
...
describes large `proprietary <./P.html#proprietary>`__ systems such
as traditional `mainframes <./M.html#mainframe>`__, installations
of SAP and Oracle, or rooms full of Windows servers remarkably well, and
the adjective is used casually. Compare
`Shub-Internet <./S.html#Shub-Internet>`__ and `crawling
horror <crawling-horror>`__.
...
`Multics <./M.html#Multics>`__, `Unix <./U.html#Unix>`__, and
`ITS <./I.html#ITS>`__. The name `ITS <./I.html#ITS>`__
...
`timesharing <./T.html#timesharing>`__
...
about a `nastygram <./N.html#nastygram>`__! He must've had his cursor
...
[WPI: from the `DEC <./D.html#DEC>`__ abbreviation CUSP, for
...
well to users is cuspy. Oppose `rude <./R.html#rude>`__.
...
storage medium. See also `burn a CD <./B.html#burn-a-CD>`__.
...
`MEGO <./M.html#MEGO>`__ factor. The computer equivalent of
bureaucratese.
...
See `cyberspace <cyberspace>`__, `ice <./I.html#ice>`__,
`jack in <./J.html#jack-in>`__, `go
flatline <./G.html#go-flatline>`__.
...
prop of `cyberpunk <cyberpunk>`__ SF. Serious efforts to
construct `virtual reality <./V.html#virtual-reality>`__ interfaces
...
network <./T.html#the-network>`__).

2. The Internet or `Matrix <./M.html#Matrix>`__ (sense #2) as a
...
the term usually tags a `wannabee <./W.html#wannabee>`__ or outsider.
Oppose `meatspace <./M.html#meatspace>`__.

3. Occasionally, the metaphoric location of the mind of a person in
`hack mode <./H.html#hack-mode>`__. Some hackers report experiencing
...
`cycle <cycle>`__. The jargon meaning comes from the observation
...
3. vt. Syn. `bounce <./B.html#bounce>`__ (sense 4), from the phrase
...
See `wheel of reincarnation <./W.html#wheel-of-reincarnation>`__.
...
[from `cyberpunk <cyberpunk>`__] Someone interested in the uses
